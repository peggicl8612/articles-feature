...
watch(() => props.article, (newVal) => {
  if (newVal && Object.keys(newVal).length > 0) {
    currentEditId.value = newVal._id || ''
...
const schema = yup.object({
  title: yup
    .string()
    .required(t('api.articleTitleRequired')),
  content: yup
    .string()
    .required(t('api.articleContentRequired'))
})
...
const submit = handleSubmit(async (values) => {
  ...
  // 新增文章時必須上傳圖片
  if (currentEditId.value.length === 0 && fileRecords.value.length === 0) {
    createSnackbar({
  ...
  try {
   ...
   if (fileRecords.value.length > 0 && fileRecords.value[0].source !== 'server') {
    ...
    if (currentEditId.value.length > 0) {
